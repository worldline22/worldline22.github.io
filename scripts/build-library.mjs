...
const arrow = (x,y,x2,y2) => line(x,y,x2,y2)+ (x===x2 ? `<path d="M${x2-4} ${y2-6} L${x2} ${y2} L${x2+4} ${y2-6}" class="sv-line"/>` : `<path d="M${x2-6} ${y2-4} L${x2} ${y2} L${x2-6} ${y2+4}" class="sv-line"/>`);
function visual(id) {
...
const collectionTabs = current => `<div class="reading-route" aria-label="Project collections"><a href="../torch-helion/index.html" ${current==='torch'?'aria-current="page"':''}>Torch-Helion · 6 papers</a><a href="../abcd/index.html" ${current==='abcd'?'aria-current="page"':''}>ABCD · 0 papers</a></div>`;
...
const card = (p,i) => `<a class="reading-card" data-paper data-category="${p.category}" data-search="${esc([p.name,p.title,p.topic,p.category,p.hook].join(' ').toLowerCase())}" href="./${p.id}.html"><div class="card-art">${visual(p.id)}</div><div class="reading-copy"><span class="kicker">0${i+1} / ${p.topic}</span><h3>${p.name}</h3><p class="paper-hook">${p.hook}</p><div class="card-end"><span>${p.venue}<br>${p.time} min · Interactive example</span><b aria-hidden="true">↗</b></div></div></a>`;
const torch = shell({title:'Torch-Helion',description:'Six beginner-friendly visual guides to tensor compilers, data movement, fusion, layouts, and scheduling.',body:`
${bread('Torch-Helion')}${collectionTabs('torch')}<section class="library-hero"><div><span class="kicker">Project shelf 01 / Visual reading guides</span><h1>Torch-Helion<span style="color:var(--blue)">.</span></h1><p>Six ways to make tensor programs do less unnecessary work. Small examples first; compiler ideas follow.</p><div class="lib-meta"><span><b>06</b> papers</span><span><b>06</b> interactive examples</span><span>Beginner friendly</span></div></div>${heroArt('overview','The shared question','Where does the data go?')}</section>
...
<div class="shelf-heading"><h2>On the reading desk.</h2><small>Open a paper to explore</small></div><div class="shelf-tools" data-enhanced hidden><label>Find a paper or idea<input type="search" id="paper-search" placeholder="Try “layout”, “memory”, or “fusion”…" autocomplete="off"></label><div class="shelf-filters" role="group" aria-label="Filter papers by idea">${[['all','All ideas'],['movement','Data movement'],['fusion','Fusion'],['planning','Planning']].map(([id,label])=>`<button data-category-filter="${id}" aria-pressed="${id==='all'}">${label}</button>`).join('')}</div></div><p class="search-status" id="search-status" aria-live="polite">6 papers in this collection</p><section class="paper-grid" aria-label="Torch-Helion papers">${papers.map(card).join('')}</section><p id="no-results" class="notice" hidden>No matching papers. Try a broader term or select All ideas.</p>
...
function article(p,index) {
  ...
  <section class="lesson" aria-labelledby="contribution">${sectionHeading('03','contribution','What did the paper add?')}<div class="method-grid">${p.methods.map(([title,body],i)=>`<div class="method-card"><span class="step-number">0${i+1}</span><h3>${title}</h3><p>${body}</p></div>`).join('')}</div><p class="contribution"><strong>The contribution:</strong> ${p.contribution}</p><a class="inline-link" style="font-size:12px" href="${pdf}#page=${p.sourceSections[0][1]}" target="_blank" rel="noopener">Follow the idea in the source paper ↗</a></section>
