...
import { abcdPapers } from './library-abcd-content.mjs';
import { abcdVisual } from './library-abcd-visuals.mjs';
...
function visual(id) {
  const abcdArt=abcdVisual(id,{svg,text,box,line,arrow});
  if(abcdArt) return abcdArt;
...
const bread = (current,isPaper=false,collectionName='Torch-Helion') => `<div class="breadcrumbs" aria-label="Breadcrumb"><a href="../index.html">Library</a><span aria-hidden="true">/</span>${isPaper?`<a href="./index.html">${collectionName}</a><span aria-hidden="true">/</span>`:''}<span>${esc(current)}</span></div>`;
const collectionTabs = current => `<div class="reading-route" aria-label="Project collections"><a href="../torch-helion/index.html" ${current==='torch'?'aria-current="page"':''}>Torch-Helion · ${papers.length} papers</a><a href="../abcd/index.html" ${current==='abcd'?'aria-current="page"':''}>ABCD · ${abcdPapers.length} papers</a></div>`;
...
<section class="library-hero library-overview"><div><span class="kicker">The research library / 01</span><h1>Ideas worth<br><em>building on.</em></h1><p>Papers connected to the projects I’m building. Explore the big idea, play with a small example, and follow the trail back to the source.</p><div class="lib-meta"><span><b>02</b> project shelves</span><span><b>${String(papers.length+abcdPapers.length).padStart(2,'0')}</b> visual explainers</span><span>Start with curiosity</span></div></div></section>
<div class="shelf-heading"><h2>Choose a project.</h2><small>A home for the ideas behind the work</small></div>
<section class="collection-grid" aria-label="Project shelves"><a class="collection" href="./torch-helion/index.html"><span class="collection-number">01 / Compiler systems</span><h3>Torch-Helion</h3><p>Move less data. Find better schedules. Explore the decisions that make tensor programs efficient.</p><div class="mini-map">${visual('vtc')}</div><div class="collection-footer"><span>${papers.length} papers · Diagrams + interactive examples</span><span>Explore shelf ↗</span></div></a><a class="collection" href="./abcd/index.html"><span class="collection-number">02 / Hardware design</span><h3>ABCD</h3><p>Describe the architecture. Map the workload. Check the result. Explore LLM-assisted hardware design and the compiler foundations behind it.</p><div class="mini-map">${visual('calyx')}</div><div class="collection-footer"><span>${abcdPapers.length} papers · Diagrams + interactive examples</span><span>Explore shelf ↗</span></div></a></section>
...
const abcdPrimer=`<section class="primer" id="basics"><div><span class="kicker">A 60-second starting point</span><h2>A chip needs a plan, a program, and a reality check.</h2><p>The papers connect these pieces at different levels.</p></div><dl class="primer-terms"><div><dt>RTL</dt><dd>A hardware description of registers and the logic that updates them.</dd></div><div><dt>Hierarchy / module</dt><dd>A design built from smaller blocks with defined connections.</dd></div><div><dt>Mapping</dt><dd>Assigning a workload’s operations and data to hardware resources.</dd></div><div><dt>PPA</dt><dd>Power, performance, and area: physical costs and speed of an implementation.</dd></div><div><dt>SoC / CGRA</dt><dd>A system-on-chip / an array of configurable arithmetic units.</dd></div><div><dt>IR / synthesis</dt><dd>A structured compiler representation / turning a hardware description into a circuit.</dd></div></dl></section>`;
const abcd = shell({title:'ABCD',description:'Visual guides to LLM-assisted hardware design, hardware/software co-design, PPA optimization, and accelerator compiler foundations.',body:`
${bread('ABCD')}${collectionTabs('abcd')}<section class="library-hero"><div><span class="kicker">Project shelf 02 / Visual reading guides</span><h1>ABCD<span style="color:var(--blue)">.</span></h1><p>From a hardware idea to a working system. Explore hierarchy, executable mapping, and physical design costs through small, concrete examples.</p><div class="lib-meta"><span><b>${abcdPapers.length}</b> papers</span><span><b>${abcdPapers.length}</b> interactive examples</span><span>Beginner friendly</span></div></div>${heroArt('abcd-overview','The shared question','How do the pieces work together?')}</section>
<section class="priority-path" aria-labelledby="reading-order"><span class="kicker">Suggested starting path</span><h2 id="reading-order">Start with these four.</h2><p>Read the plain-language takeaway, then try the example. Together, these guides introduce hierarchy, co-design, full-system evaluation, and PPA feedback.</p><div class="reading-route">${abcdPapers.filter(p=>p.priority).map((p,i)=>`${i?'<span aria-hidden="true">→</span>':''}<a href="./${p.id}.html">${p.name}</a>`).join('')}</div></section>
${abcdPrimer}<section aria-label="Compiler foundations reading path"><span class="kicker">Then revisit the compiler and architecture foundations</span><div class="reading-route">${abcdPapers.filter(p=>p.category==='foundations').map(p=>`<a href="./${p.id}.html">${p.name}</a>`).join('')}</div></section>
<div class="shelf-heading"><h2>On the reading desk.</h2><small>Open a paper to explore</small></div><div class="shelf-tools" data-enhanced hidden><label>Find a paper or idea<input type="search" id="paper-search" placeholder="Try “hierarchy”, “mapping”, or “PPA”…" autocomplete="off"></label><div class="shelf-filters" role="group" aria-label="Filter papers by idea">${[['all','All ideas'],['hierarchy','Hierarchy'],['codesign','Co-design'],['ppa','PPA optimization'],['foundations','Foundations']].map(([id,label])=>`<button data-category-filter="${id}" aria-pressed="${id==='all'}">${label}</button>`).join('')}</div></div><p class="search-status" id="search-status" aria-live="polite">${abcdPapers.length} papers in this collection</p><section class="paper-grid" aria-label="ABCD papers">${abcdPapers.map(card).join('')}</section><p id="no-results" class="notice" hidden>No matching papers. Try a broader term or select All ideas.</p>
<section class="connections"><div class="shelf-heading"><h2>From reading to building.</h2><small>Possible connections to ABCD</small></div><div class="connection-grid">${abcdPapers.map(p=>`<a href="./${p.id}.html#project"><strong>${p.name} ↗</strong><span>${p.bridge[0]} → ${p.bridge[1]}</span></a>`).join('')}</div><p class="collection-note">These connections suggest comparisons and experiments. They do not establish novelty or claim that ABCD implements the papers.</p></section>`});
...
function article(p,index,collectionName='Torch-Helion',collectionPapers=papers) {
  const pdf=p.pdfUrl;
  const sourceLink=page=>page?`${pdf}#page=${page}`:p.sourceUrl;
  const related=collectionPapers.find(x=>x.id===p.related);
  const previous=collectionPapers[(index+collectionPapers.length-1)%collectionPapers.length];
  const next=collectionPapers[(index+1)%collectionPapers.length];
  return shell({title:p.name,description:p.takeaway,body:`${bread(p.name,true,collectionName)}
...
for(const [index,paper] of abcdPapers.entries()) await writeFile(`site/library/abcd/${paper.id}.html`,article(paper,index,'ABCD',abcdPapers));
console.log(`Generated Library, two project shelves, and ${papers.length+abcdPapers.length} standalone paper explainers.`);
