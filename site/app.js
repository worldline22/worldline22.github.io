...
function home() {
  return `<section class="hero"><div><div class="eyebrow">Computer architecture & AI systems</div><h1>Yuchao Qin</h1><p class="role">Ph.D. student <span>· National University of Singapore</span></p><p class="intro">I work at the intersection of <strong>computer architecture, compilers, and AI systems</strong> — exploring how better hardware and software can make intelligent computing more efficient.</p><p class="intro">I am advised by <strong>Prof. Tulika Mitra</strong> at NUS. I studied Applied Physics at Peking University. My research experience also includes Stanford and ByteDance Seed.</p><div class="links"><a class="button primary" href="./assets/yuchaoCV.pdf" target="_blank" rel="noopener">Curriculum vitae <span aria-hidden="true">↗</span></a><a href="mailto:qinyuchao@stu.pku.edu.cn">Email ↗</a><a href="https://github.com/worldline22" target="_blank" rel="noopener">GitHub ↗</a></div></div><div class="visual"><canvas id="research-graph" role="img" aria-label="Animated three-dimensional computation graph with connected layers of nodes"></canvas><span class="visual-label">FIG. 01 / CONNECTED COMPUTATION</span><button class="motion-button" id="motion-toggle" aria-label="Pause animation">Pause Ⅱ</button></div></section>
  ...
  <section class="section">${heading('01','Selected projects','#/projects','All projects')}<div class="project-grid">${data.projects.slice(0,3).map(projectCard).join('') || '<p class="empty">Projects will appear here.</p>'}</div></section>
  <section class="section">${heading('02','Selected papers','#/papers','All papers')}${data.papers.map(paperRow).join('') || '<p class="empty">Papers will appear here.</p>'}</section>
  <div class="lower-grid"><section class="section">${heading('03','Education')}<div class="education-row"><span class="school-mark">NUS</span><div><h3>National University of Singapore</h3><p>Ph.D. in Computer Science</p><small>2026–Present · Advisor: Prof. Tulika Mitra</small></div></div><div class="education-row"><span class="school-mark">PKU</span><div><h3>Peking University</h3><p>B.S. in Applied Physics</p><small>2022–2026 · Department of EECS</small></div></div><p class="about-personal">Beyond research, I enjoy football. At Peking University, I captained the EECS soccer team.</p></section><section class="section">${heading('04','From the notebook','#/notes','All notes')}${data.notes.slice(0,2).map(noteRow).join('') || emptyNotes}</section></div>`;
}
...
function render() {
  ...
  const page = parts[0] === 'main' ? 'home' : parts[0] || 'home';
  ...
  document.title = `${page === 'home' ? 'Yuchao Qin' : page[0].toUpperCase()+page.slice(1)+' · Yuchao Qin'} · Computer Architecture & AI Systems`;
  ...
  else if(page==='studio') main.innerHTML=`<section class="detail"><div class="section-index">Owner access</div><h1>Private workspace.</h1><p class="lead">A quiet space for work in progress.</p>${config.apiBase?`<p>Sign in with your GitHub account to manage projects and notes.</p><a class="button primary" href="${esc(config.apiBase)}/auth/login">Continue with GitHub ↗</a>`:`<div class="notice">The private workspace is waiting for its Cloudflare connection. Private content is not available on this public site.</div>`}</section>`;
  ...
  const canvas=document.querySelector('canvas');if(canvas)cleanup=startGraph(canvas);
}
