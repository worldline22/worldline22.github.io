...
export function startNeural(canvas) {
  ...
  const hero = canvas.closest('.motion-scene');
  const button = hero.querySelector('[data-motion]');
  ...
  function draw() {
    ...
    const cx=width*.5+smoothX*9;
    const cy=height*.44+smoothY*7;
    const scale=Math.min(width*.15,height*.22);
